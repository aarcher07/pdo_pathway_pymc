...
def likelihood_fwd(params, fwd_rtol = 1e-8, fwd_atol=1e-8, mxsteps=int(1e4)):
    ...
    for exp_cond in ['dAJ-L']:#['WT-L', 'dAJ-L', 'dD-L', 'dP-L']:
        ...
        try:
            ...
            lik -= 0.5*(((TIME_SERIES_MEAN[exp_cond] - yout[:, DATA_INDEX])/np.array([1,0.01,1,0.1]))**2).to_numpy().sum()#TIME_SERIES_STD[exp_cond])**2).to_numpy().sum()
...
def likelihood_derivative_fwd(params, fwd_rtol = 1e-8, fwd_atol=1e-8, mxsteps=int(1e4)):
    ...
    for exp_cond in ['dAJ-L']:#['WT-L', 'dAJ-L', 'dD-L', 'dP-L']:
        ...
        try:
            ...
            grads[:,DATA_INDEX]  = (TIME_SERIES_MEAN[exp_cond] - yout[:, DATA_INDEX])/np.array([1,0.01,1,0.1])**2#(TIME_SERIES_STD[exp_cond])**2
