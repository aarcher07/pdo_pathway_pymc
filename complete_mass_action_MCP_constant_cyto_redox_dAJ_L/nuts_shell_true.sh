...
#SBATCH --job-name=pdu_constant_cyto_dAJ_true_%A_%a
#SBATCH --output=out/pdu_constant_cyto_dAJ_true_out_%A_%a
#SBATCH --error=err/pdu_constant_cyto_dAJ_true_err_%A_%a
...
python sample_true.py "${nsamples[(($zero_index / $sublen_init_burn_in_acc_rate) % $len_nsamples)]}" "${burn_in[($zero_index / $len_init) % $len_burn_in]}" "${nchains}" "${acc_rate[($zero_index / $sublen_init_burn_in) % $len_acc_rate]}" "${fwd_rtol[(($zero_index / $sublen_init_burn_in_acc_rate_nsamples) % $len_fwd_rtol)]}" "${fwd_atol[(($zero_index / $sublen_init_burn_in_acc_rate_nsamples_fwd_rtol) % $len_fwd_atol)]}" "${bck_rtol[(($zero_index / $sublen_init_burn_in_acc_rate_nsamples_fwd_rtol_fwd_atol) % $len_bck_rtol)]}" "${bck_atol[(($zero_index / $sublen_init_burn_in_acc_rate_nsamples_fwd_rtol_fwd_atol_bck_rtol) % $len_bck_atol)]}" "${fwd_mxsteps}" "${bck_mxsteps}" "${init[$zero_index % $len_init]}"
