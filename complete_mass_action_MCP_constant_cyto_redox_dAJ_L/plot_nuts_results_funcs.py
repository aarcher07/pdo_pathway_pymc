...
def plot_time_series_distribution_post(samples, plot_file_location, nchains, fwd_atol, fwd_rtol, mxsteps):
    lib.CVodeSStolerances(solver_dAJ._ode, fwd_rtol, fwd_atol)
    lib.CVodeSetMaxNumSteps(solver_dAJ._ode, int(mxsteps))

    c = ['r', 'y', 'b', 'g', 'k']
    exp_cond = 'dAJ-L'
    param_list = [*LOCAL_DEV_PARAMETER_LIST[:-4], *[param + '_dAJ' for param in LOCAL_DEV_PARAMETER_LIST[-4:]]]
    solver = solver_dAJ
    ...
    for chain_ind in range(min(5, nchains)):
        ...
        for j in range(0,dataarray.shape[0],int(dataarray.shape[0]/500)):
            ...
            POLAR_VOLUME = (4./3.)*np.pi*((10 ** param_samples_copy[LOCAL_PARAMETER_LIST.index('AJ_radius')]) ** 3)
            y0['NADH_MCP'] = (10 ** (param_samples_copy[LOCAL_PARAMETER_LIST.index('NADH_NAD_TOTAL_CYTO')]
                                     + param_samples_copy[LOCAL_PARAMETER_LIST.index('NADH_NAD_RATIO_CYTO')])) / (
                                     10 ** param_samples_copy[LOCAL_PARAMETER_LIST.index('NADH_NAD_RATIO_CYTO')] + 1)
            y0['NAD_MCP'] = 10 ** param_samples_copy[LOCAL_PARAMETER_LIST.index('NADH_NAD_TOTAL_CYTO')] / (
                    10 ** param_samples_copy[LOCAL_PARAMETER_LIST.index('NADH_NAD_RATIO_CYTO')] + 1)
            y0['PduCDE'] = (10 ** (param_samples_copy[LOCAL_PARAMETER_LIST.index('nPduCDE')] + param_samples_copy[
                LOCAL_PARAMETER_LIST.index('nMCPs')])) / (Avogadro * POLAR_VOLUME)
            y0['PduP'] = 10 ** (param_samples_copy[LOCAL_PARAMETER_LIST.index('nPduP')] + param_samples_copy[
                LOCAL_PARAMETER_LIST.index('nMCPs')]) / (Avogadro * POLAR_VOLUME)
            y0['PduQ'] = 10 ** (param_samples_copy[LOCAL_PARAMETER_LIST.index('nPduQ')] + param_samples_copy[
                LOCAL_PARAMETER_LIST.index('nMCPs')]) / (Avogadro * POLAR_VOLUME)
            y0['PduL'] = 10 ** (param_samples_copy[LOCAL_PARAMETER_LIST.index('nPduL')] + param_samples_copy[
                LOCAL_PARAMETER_LIST.index('nMCPs')]) / (Avogadro * POLAR_VOLUME)
            y0['OD'] = 10 ** param_samples_copy[LOCAL_PARAMETER_LIST.index('A')]

            params_dict = {param_name : param_val for param_val,param_name in zip(param_samples_copy, LOCAL_PARAMETER_LIST)}
    ...
    ax[3, chain_ind].set_title('OD Time Series')

    plt.suptitle('Experimental Condition ' + str(exp_cond))
    fig.tight_layout()
    plt.savefig(os.path.join(plot_file_location, 'time_series_results_' + str(exp_cond) + '_post.png'))

def plot_time_series_distribution_prior(samples, plot_file_location, fwd_atol, fwd_rtol, mxsteps):
    lib.CVodeSStolerances(solver_dAJ._ode, fwd_rtol, fwd_atol)
    lib.CVodeSetMaxNumSteps(solver_dAJ._ode, int(mxsteps))
    ...
    fig, ax = plt.subplots(4, figsize=(15,15)) #min(5, post_nchains))
    dataarray = samples.prior.to_dataframe()
    dataarray = dataarray[param_list]
    for jj in range(4):
        ax[jj].scatter(TIME_SAMPLES, TIME_SERIES_MEAN[exp_cond].iloc[:,jj])
    ...
    ax[1].set_title('3-HPA Time Series')
    ax[2].set_title('1,3-PD Time Series')
    ax[3].set_title('OD Time Series')

    plt.suptitle('Experimental Condition ' + str(exp_cond))
    fig.tight_layout()
    plt.savefig(os.path.join(plot_file_location, 'time_series_results_' + str(exp_cond) + '_prior.png'))
...
def plot_corr(data, directory_plot, nchains, thres=5e-2):
    for chain_ind in range(nchains):
        ...
        fig.set_size_inches(30, 40, forward=True)
...
def prior_to_post_map(data_post, data_prior, directory_plot, nchains):
    data_prior = data_prior.prior.to_dataframe()
    plot_file_location = os.path.join(directory_plot, 'prior_to_post')
    Path(plot_file_location).mkdir(parents=True, exist_ok=True)

    for param_name in PLOT_PARAMETERS:
        fig, ax = plt.subplots(min(5, nchains), figsize=(10, 10))  # min(5, post_nchains))
        data_prior_param = data_prior[param_name]

        for chain_ind in range(nchains):
            data_post_array = data_post.posterior.to_dataframe().loc[[chain_ind]]
            data_post_param = data_post_array[param_name]
            ax[chain_ind].hist(data_prior_param,bins=25, label='prior', color='red', alpha = 0.5, density = 'True')
            ax[chain_ind].hist(data_post_param,bins=25, label='posterior', color='blue', alpha = 0.5, density = 'True')
            ax[chain_ind].set_title('Chain ' + str(chain_ind))
            ax[chain_ind].legend()
            ax[chain_ind].set_ylabel('Probability Density')
            ax[chain_ind].set_xlabel(MODEL_PARAMS_TO_TEX[param_name] + ' ' + MODEL_PARAMS_TO_UNITS[param_name])

        plt.suptitle(MODEL_PARAMS_TO_TEX[param_name], fontsize = 25)
        file_name = os.path.join(plot_file_location, param_name)
