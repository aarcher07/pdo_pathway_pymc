...
for exp_ind, gly_cond in enumerate([50,60,70,80]):
    ...
    for j,param in enumerate(PARAMETER_LIST):
        ...
        if param == 'G_EXT_INIT':
            lik_dev_params[N_MODEL_PARAMETERS + exp_ind]= lik_dev_param.sum()
        elif param in ['L','k','t0']:
            jj = ['L','k','t0'].index(param)
            lik_dev_params[N_MODEL_PARAMETERS + 4 + exp_ind*N_DCW_PARAMETERS + jj ]= lik_dev_param.sum()
        else:
            lik_dev_params[j] = lik_dev_param.sum()
