...
def RHS(t, x, params):
    ...
    PermCellGlycerol = 10**params.PermCellGlycerol
...
for exp_ind, gly_cond in enumerate([50,60,70,80]):
    ...
    param_sample[PARAMETER_LIST.index('k')] = param_sample[PARAMETER_LIST.index('k')]/HRS_TO_SECS
    param_sample[PARAMETER_LIST.index('t0')] = param_sample[PARAMETER_LIST.index('t0')]*HRS_TO_SECS
    tvals = np.linspace(0, (TIME_SAMPLES[gly_cond][-1] + 10)*HRS_TO_SECS)
    ...
    y0['dcw'] =  param_sample[PARAMETER_LIST.index('L')]/(1+np.exp(param_sample[PARAMETER_LIST.index('k')]*param_sample[PARAMETER_LIST.index('t0')]))
    ...
    # time_err = [22,50,20,21]
    # for var in VARIABLE_NAMES:
    #     plt.plot(tvals/HRS_TO_SECS,output.view(problem.state_dtype)[var])
    #     # plt.axvline(x=time_err[exp_ind]/HRS_TO_SECS)
    #     plt.show()
