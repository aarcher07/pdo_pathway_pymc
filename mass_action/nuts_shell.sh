...
nsamples=(2 3)#(5e3 7e3 1e4)
burn_in=(1 2)#(3e3 4e3 6e3)
nchains=5
acc_rate=(0.8)
atol=(1e-10)
rtol=(1e-8)
mxsteps=1e5
init=("jitter+adapt_diag" "jitter+adapt_full")
...
python pymc_funcs.py "${nsamples[(($zero_index / $sublen_burn_in_acc_rate) % $len_nsamples)]}" "${burn_in[$zero_index % $len_burn_in]}" "${nchains}" "${acc_rate[($zero_index / $len_burn_in) % $len_acc_rate]}" "${atol[(($zero_index / $sublen_burn_in_acc_rate_nsamples) % $len_atol)]}" "${rtol[(($zero_index / $sublen_burn_in_acc_rate_nsamples_atol) % $len_rtol)]}" "${mxsteps}" "${init[(($zero_index / $sublen_burn_in_acc_rate_nsamples_atol_rtol) % $len_init)]}"