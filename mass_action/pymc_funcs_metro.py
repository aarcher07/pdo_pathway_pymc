...
from likelihood_funcs_adj_metro import likelihood_adj, likelihood_derivative_adj  #TODO : change to _3HPA
from os.path import dirname, abspath
...
def sample(nsamples, burn_in, nchains, acc_rate=0.8, atol=1e-8, rtol=1e-8, mxsteps=int(2e4),
    ...
    with pm.Model():
        ...
        nuts_vars = [*permeability_params, *kinetic_params, *enzyme_init]

        all_vars = [*nuts_vars, *gly_init]
        # convert m and c to a tensor vector
        theta = at.as_tensor_variable(all_vars)
        # use a Potential to "call" the Op and include it in the logp computation
        pm.Potential("likelihood", logl(theta))
        idata_nuts = pm.sample(draws=int(nsamples), init=init, cores=nchains, chains=nchains, tune=int(burn_in),
                               step=[pm.NUTS(nuts_vars, target_accept=acc_rate), pm.Metropolis(gly_init)],
